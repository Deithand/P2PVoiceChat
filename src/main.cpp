                      ...
                      void *userData)
{
    ...
    if (incoming_queue.try_pop(encoded_packet)) {
        SAMPLE decoded_samples[framesPerBuffer * NUM_CHANNELS];
        int decoded_frames = opus_decode_float(decoder, encoded_packet.data(), encoded_packet.size(), (SAMPLE*)outputBuffer, framesPerBuffer, 0);
        if (decoded_frames < 0) {
            std::cerr << "Opus decode failed: " << opus_strerror(decoded_frames) << std::endl;
            // Fill with silence on error
            for (size_t i = 0; i < framesPerBuffer * NUM_CHANNELS; ++i) ((SAMPLE*)outputBuffer)[i] = 0;
        }
    } else {
    ...
    if (inputBuffer != NULL) {
        ...
        opus_int32 encoded_bytes = opus_encode_float(encoder, (const SAMPLE*)inputBuffer, framesPerBuffer, new_packet.data(), new_packet.size());
        if (encoded_bytes < 0) {
            std::cerr << "Opus encode failed: " << opus_strerror(encoded_bytes) << std::endl;
        } else {
...
void network_receive_loop(int sockfd) {
    ...
    int from_port;
    while (keep_running) {
        ssize_t n = network_receive(sockfd, buffer, MAX_PACKET_SIZE, from_ip, from_port);
        if (n > 0) {
...
void network_send_loop(int sockfd, const std::string& peer_ip, int peer_port) {
    ...
    while (keep_running) {
        if (outgoing_queue.try_pop(p)) {
            network_send(sockfd, peer_ip, peer_port, (const char*)p.data(), p.size());
        } else {
...
int main(int argc, char *argv[]) {
    ...
    // --- Init Opus ---
    int opus_error;
    encoder = opus_encoder_create(SAMPLE_RATE, NUM_CHANNELS, OPUS_APPLICATION_VOIP, &opus_error);
    if (opus_error != OPUS_OK) { std::cerr << "Failed to create Opus encoder" << std::endl; return 1; }
    opus_encoder_ctl(encoder, OPUS_SET_BITRATE(OPUS_BITRATE));

    decoder = opus_decoder_create(SAMPLE_RATE, NUM_CHANNELS, &opus_error);
    if (opus_error != OPUS_OK) { std::cerr << "Failed to create Opus decoder" << std::endl; return 1; }
    ...
    // --- Init PortAudio ---
    PaStream *stream;
    if (Pa_Initialize() != paNoError) { std::cerr << "PortAudio init failed." << std::endl; return 1; }
    if (Pa_OpenDefaultStream(&stream, NUM_CHANNELS, NUM_CHANNELS, paFloat32, SAMPLE_RATE, FRAMES_PER_BUFFER, paCallback, NULL) != paNoError) {
        std::cerr << "PortAudio open stream failed." << std::endl; return 1;
    }
    ...
    if (Pa_StartStream(stream) != paNoError) { std::cerr << "PortAudio start stream failed." << std::endl; return 1; }
